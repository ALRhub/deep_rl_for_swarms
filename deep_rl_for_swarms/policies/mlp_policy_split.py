    ...
    def _init(self, ob_space, ac_space, hid_size, gaussian_fixed_var=True):
        ...
        with tf.variable_scope('vf'):
            with tf.variable_scope('me'):
                me_v = tf.layers.dense(flat_obs_input_layer, hid_size[0], name="fc0",
                                           kernel_initializer=U.normc_initializer(1.0))
            last_out = tf.concat([me_v, flat_feature_input_layer], axis=1)
            for i in range(num_hid_layers - 1):
                last_out = tf.layers.dense(last_out, hid_size[i+1], name="fc%i" % (i + 1),
                                           kernel_initializer=U.normc_initializer(1.0))
                if self.layer_norm:
        ...
        with tf.variable_scope('pol'):
            with tf.variable_scope('me'):
                me_pi = tf.layers.dense(flat_obs_input_layer, hid_size[0], name="fc0",
                                           kernel_initializer=U.normc_initializer(1.0))
            last_out = tf.concat([me_pi, flat_feature_input_layer], axis=1)
